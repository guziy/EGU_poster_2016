...
\begin{tikzpicture}
    ...
    \node[diamond, text width=0.05\linewidth, align=center, draw] (operation) at (10,-9) {\Huge -};

    \node[label={\small CanESM2-CRCM5-L minus CanESM2-CRCM5-NL}] (lcc_vs_nlcc) at (20,-19) {\includegraphics[scale=0.9]{impact_on_cc}};
    ...
    \node[below=1.0cm of point4, draw, text width=0.5\linewidth, align=left] (winter){\small \textbf{Winter streamflow.} Dipole pattern: \color{Blue}negative \color{Black} sign due to meltwater stored in lakes, \color{Red}positive \color{Black} sign since lakes supply water in frozen conditions};
    ...
    %lines streamflow
    \draw[-latex, line width=5pt] (winter.east) to[out=0,in=215] ([xshift=2em,yshift=2em]lcc_vs_nlcc.south west);
    \draw[-latex, line width=5pt] (summer.north) to[out=90,in=270] ([xshift=0.5em,yshift=1.5em]lcc_vs_nlcc.south);
